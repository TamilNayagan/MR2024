...
async def set_shortner_2(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://t.me/TG_SUPPORT_GROUP').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
